      ...
      upgrade(db, oldVersion, newVersion) {
        ...
        if (oldVersion < 2) {
          ...
          if (!db.objectStoreNames.contains('note_tags')) {
            const noteTagsStore = db.createObjectStore('note_tags', { keyPath: ['noteId', 'tagId'] });
          ...
          if (!db.objectStoreNames.contains('note_categories')) {
            const noteCatsStore = db.createObjectStore('note_categories', { keyPath: ['noteId', 'categoryId'] });
