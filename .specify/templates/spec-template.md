...
- **FR-010**: 系统必须为面向用户的交互提供渐进式反馈（例如加载占位、分块/流式响应），以避免阻塞主线程或请求路径。
- **FR-011**: 系统必须能够为关键交互暴露可测量的响应延迟指标并将其上报到监控系统，以便在 CI/生产中检测回归。
...
- **SC-005**: 关键交互的 p95 延迟满足 spec 中定义的性能预算 (TODO: 指定具体阈值)。
- **SC-006**: 主要用户任务的可用性指标(如任务完成率)达到或优于项目设定目标 (例如 ≥ TODO%)。
- **SC-007**: 在高延迟或部分失败场景下, 系统能够提供可验证的降级行为并保留可辨识的错误/进度反馈。
