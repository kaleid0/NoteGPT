...
- [ ] T009 设置环境配置管理
- [ ] T009a 定义并编写可用性测试脚本/任务 (包括任务完成率与明确验收标准)
- [ ] T009b 配置性能基准与自动化测试（定义 p95/p99 测试脚本并记录基线）
- [ ] T009c 在监控系统中配置延迟/错误率/吞吐的仪表盘与告警
- [ ] T009d 设计并实现支持流式/分块响应的 API 合约与契约测试
- [ ] T009e 实施后端代理服务 (API Key 管理、最小权限、审计日志) 并提供本地部署指南
- [ ] T009f 提供 OpenAI 示例实现与演示脚本（含流式处理示例）
- [ ] T009g 在 quickstart 中加入 API Key 配置与安全注意事项
