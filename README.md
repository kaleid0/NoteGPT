# NoteGPT 📝✨

**NoteGPT** 是一款集成 AI 辅助写作功能的现代笔记应用。支持多种 LLM 提供商，采用流式输出技术，提供流畅的 AI 写作体验。

![Version](https://img.shields.io/badge/version-0.1.0-blue.svg)
![License](https://img.shields.io/badge/license-MIT-green.svg)
![Node](https://img.shields.io/badge/node-%3E%3D18.0.0-brightgreen.svg)

---

## ✨ 功能特性

### 📒 笔记管理
- **完整的 CRUD 操作** - 创建、编辑、删除笔记
- **本地优先** - 使用 IndexedDB 本地存储，无需注册账号
- **自动保存** - 编辑时自动保存，防止数据丢失
- **实时同步** - WebSocket 支持多设备实时协作

### 🤖 AI 辅助写作
- **流式输出** - 使用 Server-Sent Events (SSE) 实现，首字符延迟仅 ~170ms
- **智能选择** - 支持选中文本或全文的 AI 改进
- **多提供商支持**:
  - OpenAI (GPT-3.5-turbo, GPT-4)
  - DeepSeek
  - 百炼 (Bailian)
  - 自定义提供商

### ⚙️ 高度可配置
- **自定义 Prompt 模板** - 支持 `{{input}}` 占位符
- **本地持久化** - 配置保存在 localStorage
- **预设模板库** - 开箱即用的常用模板

### 📱 响应式设计
- 移动设备 (iPhone 12: 375px)
- 平板设备 (iPad: 768px)  
- 桌面设备 (1366px+)

### ♿ 可访问性
- ARIA 标签和角色
- 完整的键盘导航支持
- 屏幕阅读器兼容
- axe 可访问性得分 100/100

---

## 🚀 快速开始

### 前置条件

- **Node.js** >= 18.0.0
- **npm** 或 **pnpm**

### 安装

```bash
# 克隆仓库
git clone https://github.com/kaleid0/NoteGPT.git
cd NoteGPT

# 安装依赖
npm install
```

### 启动开发服务器

需要同时启动前端和后端服务：

```bash
# 终端 1: 启动后端服务 (端口 4000)
cd server
npm run dev

# 终端 2: 启动前端服务 (默认端口 5173)
cd client
npm run dev
```

访问 [http://localhost:5173](http://localhost:5173) 开始使用。

### 生产构建

```bash
# 构建前端
cd client
npm run build

# 构建后端
cd server
npm run build
npm start
```

---

## 📁 项目结构

```
NoteGPT/
├── client/                 # 前端 React 应用
│   ├── src/
│   │   ├── components/     # UI 组件
│   │   │   ├── AIButton/   # AI 触发按钮
│   │   │   ├── AIStreamModal/  # AI 流式输出弹窗
│   │   │   ├── NoteEditor/ # 笔记编辑器
│   │   │   ├── NoteList/   # 笔记列表
│   │   │   └── Settings/   # 设置页面
│   │   ├── context/        # React Context
│   │   ├── hooks/          # 自定义 Hooks
│   │   ├── lib/            # 工具库
│   │   ├── pages/          # 页面组件
│   │   ├── stores/         # Zustand 状态管理
│   │   ├── styles/         # 全局样式
│   │   ├── ui/             # 基础 UI 组件
│   │   └── utils/          # 工具函数
│   └── package.json
│
├── server/                 # 后端 Fastify 服务
│   ├── src/
│   │   ├── middleware/     # 中间件 (auth, rate_limit)
│   │   ├── routes/         # API 路由
│   │   │   ├── generate.ts # AI 生成接口
│   │   │   └── sync.ts     # 同步接口
│   │   ├── services/       # 业务服务
│   │   └── index.ts        # 入口文件
│   └── package.json
│
├── shared/                 # 共享代码
│   └── sync-protocol.ts    # 同步协议定义
│
├── tests/                  # 测试文件
│   ├── e2e/                # E2E 测试 (Playwright)
│   ├── integration/        # 集成测试
│   └── unit/               # 单元测试 (Vitest)
│
└── package.json            # 根配置 (workspaces)
```

---

## 🔧 配置

### LLM 配置

在应用的 **设置** 页面中配置 AI 服务：

| 配置项 | 说明 |
|--------|------|
| Provider | LLM 提供商 (openai/deepseek/bailian/custom) |
| API Key | 服务商 API 密钥 |
| Base URL | API 基础地址 (可选) |
| Model | 模型名称 |
| Prompt Template | 自定义提示词模板 |

**Prompt 模板示例**：
```
请帮我改进以下文本，使其更加专业和易读：

{{input}}
```

### 环境变量

创建 `.env` 文件进行配置（可选）：

```env
# 服务端口
PORT=4000

# 日志级别
LOG_LEVEL=info
```

---

## 🧪 测试

```bash
# 运行单元测试
npm run test:unit

# 运行 E2E 测试
npm run test:e2e

# 在 client 目录运行测试
cd client && npm test

# 在 server 目录运行测试
cd server && npm test
```

### 测试覆盖

| 测试类型 | 状态 |
|----------|------|
| 单元测试 | ✅ 完整覆盖 |
| 集成测试 | ✅ 后端 SSE 和 LLM 适配 |
| E2E 测试 | ✅ 21/21 通过 |
| 可访问性测试 | ✅ 100/100 |

---

## 📊 性能指标

| 指标 | 值 |
|------|-----|
| AI 首字符延迟 (p95) | ~170ms |
| 总流式时间 | ~320ms |
| 吞吐量 | ~36-37 字符/秒 |
| 页面加载时间 | <1s |

---

## 🛠️ 技术栈

### 前端
- **React 18** - UI 框架
- **TypeScript 5** - 类型安全
- **Vite 5** - 构建工具
- **Zustand** - 状态管理
- **React Router 7** - 路由管理
- **IndexedDB (idb)** - 本地存储
- **CSS Modules** - 样式隔离

### 后端
- **Fastify** - Web 框架
- **WebSocket** - 实时通信
- **Better-SQLite3** - 数据库
- **ts-node-dev** - 开发服务器

### 测试
- **Vitest** - 单元测试
- **Playwright** - E2E 测试
- **Testing Library** - React 组件测试
- **axe-core** - 可访问性测试

---

## 📝 API 接口

### AI 生成

```http
POST /v1/generate
Content-Type: application/json

{
  "input": "要改进的文本",
  "config": {
    "provider": "openai",
    "apiKey": "sk-xxx",
    "model": "gpt-3.5-turbo"
  }
}
```

响应: Server-Sent Events (SSE) 流

### 同步

```http
WebSocket /v1/sync?token=xxx
```

支持实时笔记同步和协作编辑。

---

## 🤝 贡献

欢迎贡献！请遵循以下步骤：

1. Fork 本仓库
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request

### 开发规范

- 遵循 TypeScript 严格模式
- 使用 ESLint + Prettier 格式化代码
- 新功能需要添加测试
- 保持可访问性标准

---

## 📄 许可证

本项目基于 MIT 许可证开源 - 查看 [LICENSE](LICENSE) 文件了解详情。

---

## 🙏 致谢

- [React](https://react.dev/)
- [Fastify](https://www.fastify.io/)
- [OpenAI](https://openai.com/)
- [Vite](https://vitejs.dev/)

---

<p align="center">
  Made with ❤️ by <a href="https://github.com/kaleid0">kaleid0</a>
</p>
