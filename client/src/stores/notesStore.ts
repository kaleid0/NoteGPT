...
export const useNotesStore = create<NotesState>((set, get) => ({
  ...
  addCategory: async (category: Category) => {
    ...
    set((state) => ({
      ...
      categoryIds: state.categoryIds.includes(category.id)
        ? state.categoryIds
        : [category.id, ...state.categoryIds],
    }))
  },
    ...
    set((state) => {
      ...
      return {
        noteCategories: {
          ...
          [noteId]: current.filter((c) => c !== categoryId),
        },
      }
    })
  },
  ...
  getNotesByTag: (tagId: string) => {
    const state = get()
    const noteIds = Object.keys(state.noteTags).filter((nid) =>
      (state.noteTags[nid] || []).includes(tagId)
    )
  ...
  getNotesByCategory: (categoryId: string) => {
    const state = get()
    const noteIds = Object.keys(state.noteCategories).filter((nid) =>
      (state.noteCategories[nid] || []).includes(categoryId)
    )
    return noteIds.map((nid) => state.notesById[nid]).filter(Boolean)
  },
