...
- 可用性 (用户友好交互体验): 所有 P1 用户故事需包含可执行的可用性验收标准 (例如任务完成率、SUS 或可复现的可用性脚本)。在阶段 0/1 结束前需通过小规模真人测试或可用性评估。
- 实时性 (流畅交互): 关键用户操作的 p95 延迟预算需在 spec 中定义并在实现前接受。高延迟路径需支持渐进式/流式反馈。
- 降级与容错: 在高延迟或部分失败场景中必须提供降级路径并在验收测试中包含验证场景。
- 可观测性: 关键路径必须输出延迟、错误率和吞吐度量并配置告警; 性能回归需在 CI 中检测。
- 安全/合规: 安全与数据保留约束需在阶段 0 明确并在计划中列出合规检查点。
- 文档与可访问性: Quickstart 与用户文档必须反映可访问性/可用性要点并在阶段 1 完成时更新。
- 集成: 如果 Q1 选择后端代理, 阶段 1 需包含代理部署/测试步骤与安全校验点 (例如：密钥访问控制、最小权限策略、日志审计)。
